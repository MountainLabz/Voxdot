...
// In MeshConverter.cpp
void GodotVoxelMesher::addQuadToGodotMesh(GodotMeshData &godotMeshData,
		uint64_t packedQuad,
		float voxelScale,
	...
	// Retrieve metallicness and emissiveness for the current voxel type
	float metallicness = 0.0f;
	float emissiveness = 0.0f;
	if (type < voxel_type_properties.size() && voxel_type_properties[type].is_valid()) {
		metallicness = voxel_type_properties[type]->get_metallicness();
		emissiveness = voxel_type_properties[type]->get_emissiveness();
	} else {
		// Handle error: voxel type out of bounds or resource is null
		WARN_PRINT("Voxel type ID out of bounds or VoxelMaterialProperties resource is null.");
	}
	...
	for (int i = 0; i < 4; ++i) {
		...
		// Add Metallicness and Emissiveness to UV2
		godotMeshData.uvs2.push_back(metallicness);
		godotMeshData.uvs2.push_back(emissiveness);
...
//		uint64_t packedQuad,
//		float voxelScale,
//		const std::array<VoxelMaterialProperties, 256> &voxel_type_properties) const {
//	// Unpack quad data, mirroring GLSL shader exactly
//	int x = static_cast<int>((packedQuad >> X_SHIFT) & X_MASK);
//	int y = static_cast<int>((packedQuad >> Y_SHIFT) & Y_MASK);
//	int z = static_cast<int>((packedQuad >> Z_SHIFT) & Z_MASK);
...
void GodotVoxelMesher::convertQuadsToGodotMesh(GodotMeshData &outGodotMeshData,
	...
	if (meshData.vertices && meshData.vertexCount > 0) {
		for (int i = 0; i < meshData.vertexCount; ++i) {
			addQuadToGodotMesh(outGodotMeshData, (*meshData.vertices)[i], voxelScale, voxel_type_properties);
		}
	} else {
